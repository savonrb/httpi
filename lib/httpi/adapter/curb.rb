  ...
  module Adapter
    ...
    private
      ...
      def setup_auth(auth)
        case auth.type
        when :basic
          setup_http_auth(auth)
        when :digest
          setup_http_auth(auth)
        when :gssnegotiate
          setup_gssnegotiate_auth(auth)
        when :ssl
          setup_ssl_auth(auth)
        end
      end

      def setup_http_auth(auth)
        client.http_auth_types = auth.type
        client.username, client.password = auth.username, auth.password
      end

      def setup_gssnegotiate_auth(auth)
        client.http_auth_types = auth.type
